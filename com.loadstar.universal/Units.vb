Option Compare Text
Public Class Units
    Public Enum Enum_MeasurementType
        none
        force
        torque
        length
        ...
        voltage
    End Enum
    Private _unitType As Enum_MeasurementType = Enum_MeasurementType.force
    ...
    Private Sub ComputeCvFactor()
        If _unitType <> Enum_MeasurementType.temperature Then
            Dim colIndex = 0
            Dim rowIndex = 0
    ...
    Public Sub New(ByVal unitType As Enum_MeasurementType)
        _unitType = unitType
        Select Case unitType
            Case Enum_MeasurementType.length
                _unitList = New String() {"in", "mm", "mil"}
                cvfactor = New Double(,) _
                {{1, 25.4, 1000}, _
                {1 / 25.4, 1, 1000 / 25.4}, _
                {0.001, 0.001 * 25.4, 1}}
            Case Enum_MeasurementType.torque
            ...
            Case Enum_MeasurementType.pressure
            ...
            Case Enum_MeasurementType.angle
                _unitList = New String() {"deg"}
                cvfactor = New Double(,) _
                  {{1},
                 {1}}
            Case Enum_MeasurementType.voltage
                _unitList = New String() {"V"}
                cvfactor = New Double(,) _
                  {{1},
                 {1}}
            Case Enum_MeasurementType.temperature
        ...
        _outputUnits = _unitList(0)
    End Sub
    Public ReadOnly Property UnitType() As Enum_MeasurementType
        Get
            Return _unitType
        End Get
    End Property
